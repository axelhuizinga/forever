 ...
 ************/
	...
	class Bulma_MultiLevelNavwalker extends Walker_Nav_Menu {
		/**
    ...
    /**
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			#edev($item);
			#edump(bogo_get_url_with_lang($locale));
			/*edump(Bulma_Navwalker::$locale);
			if(in_array(Bulma_Navwalker::$locale, $item->bogo_locales))
				edump(bogo_get_url_with_lang($item->url,Bulma_Navwalker::$locale));
			else
			edump($item->ID.'::'.implode('|',$item->bogo_locales).' xfn:'.$item->xfn.':'.$item->url);*/
			if(in_array(Bulma_Navwalker::$locale, $item->bogo_locales)){
				#edev($item);
				#edev($output);
				$item->url = bogo_get_url_with_lang($item->url,Bulma_Navwalker::$locale);
				#edump($item->url);
			}
			$args     	 = wp_parse_args( $args, $defaults );
#edump("https://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);	
#edump($item);
